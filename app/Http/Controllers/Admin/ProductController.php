...
class ProductController extends Controller
{
    ...
    public function ledger(Request $request, $id)
    {
        ...
        $transactions = Transaction::orderBy('created_at', 'desc')->whereYear('created_at', $currentYear)->where('product_id', $id)->with(['issueItem'=>function($query){
            $query->with('issue');
        }, 
    ...
    public function lowStock(Request $request){
        if ($request->ajax()) {
            $datas = Product::orderBy('quantity', 'asc')
                            ->whereColumn('quantity', '<=', 'in_hand_quantity')
                            ->orWhere('quantity', '<=', 0)
                            ->with(['unit', 'productType']);

            $totaldata = $datas->count();
            $records = $datas->limit($request->length)
            ...
            foreach ($records as $data) {
                $result['data'][] = [
                    ...
                    'id' => $data->id,
                    'product' => $data->name,
                    'product_type' => $data->productType->type,
    ...
    public function checkRate($id){
       $product = Product::where('id', $id)
                ->with([
                    'unit', 
                    'productType', 
                    'materialOrderItems' => function($query) {
                        $query->with([
                            'materialOrder' => function($subQuery) {
                                $subQuery->with('vendor');
                            }
                        ])->orderBy('created_at', 'desc'); // Order by 'created_at' for materialOrderItems
                    }
                ])
                ->first();
        return view('admin.product.check-rate', compact('product'));
    }
