    ...
    <div class="col-12">
        <div class="page-title-box d-flex align-items-center justify-content-between">
            ...
            <div class="btn-group" role="group">
                <button id="btnGroupDrop1" type="button" class="shadow-none btn-sm btn btn-primary btn-label rounded-pill dropdown-toggle" data-bs-toggle="dropdown" aria-expanded="false">
                    <i class="align-middle bx bx-plus label-icon rounded-pill fs-16 me-2"></i> Add New Product
                </button>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                {{ html()->form('POST', route('admin.category.store'))->class('form-horizontal')->id('mainCategoryForm')->open() }}

                    <div class="parent_category_on form-group{{ $errors->has('parent_category') ? ' has-error' : '' }}">
                        {{ html()->label('Parent Category', 'parent_category') }}
                        {{ html()->select('parent_category', App\Models\Category::orderBy('name', 'asc')->where('parent', null)->pluck('name', 'id'))
                            ->placeholder('Select Parent Category')
                            ->class('form-control refresh-cat')
                            ->attribute('readonly') }}
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                {{ html()->form('put', route('admin.'.request()->segment(2).'.update', ''))->id('updateCategoryForm')->open() }}

                    <div class="edit_parent_category_on form-group{{ $errors->has('edit_parent_category') ? ' has-error' : '' }}">
                        {{ html()->label('Parent Category', 'edit_parent_category') }}
                        {{ html()->select('edit_parent_category', App\Models\Category::orderBy('name', 'asc')->where('parent', null)->pluck('name', 'id'))
                            ->placeholder('Select Parent Category')
                            ->class('form-control refresh-cat')
                            ->attribute('readonly') }}
                    ...
                    <div class="form-group{{ $errors->has('edit_category_name') ? ' has-error' : '' }}">
                        {{ html()->label('Category Name', 'edit_category_name') }}
                        {{ html()->text('edit_category_name')
                            ->class('form-control')
                            ->placeholder('Enter Category Name')
                            ->required() }}
                        <small class="text-danger">{{ $errors->first('edit_category_name') }}</small>
                    </div>
                    {{ html()->hidden('id', 'value')->id('editCat') }}
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                {{ html()->form('put', route('admin.'.request()->segment(2).'.update', ''))->id('makeCategoryForm')->open() }}
                    ...
                    <div class="form-group{{ $errors->has('child_category_name') ? ' has-error' : '' }}">
                        {{ html()->label('Child Category Name', 'child_category_name') }}
                        {{ html()->text('child_category_name')
                            ->class('form-control')
                            ->placeholder('Enter Category Name')
                            ->required()
                            ->attribute('readonly') }}
                        {{ html()->hidden('child_category_id', 'value')->id('child-category-id') }}
...
<div class="border-0 offcanvas offcanvas-end" tabindex="-1" id="offcanvasTop" aria-labelledby="offcanvasTopLabel">
    
    <div class="p-3 d-flex align-items-center bg-success bg-gradient offcanvas-header">
        <h5 class="m-0 text-white me-2">Product Filters</h5>
...
{{-- 
        <div class="card">
            <div class="pt-2 pb-2 card-header">
                <h6 class="mb-0 card-title">Filter By Date</h6>
            </div>
        ...
        <div class="card">
            <div class="pt-2 pb-2 card-header">
                <h6 class="mb-0 card-title">Paper Type</h6>
            </div>
        
            <div class="card-body">
                <div class="form-group{{ $errors->has('paper_type') ? ' has-error' : '' }}">
                    {{ html()->label('Paper Type', 'paper_type') }}
                    {{ html()->select('paper_type', [1 => 'White', 2 => 'Yellow', 3 => 'White Back', 4 => 'Gray Back'])
                        ->id('filter_paper_type')
        ...
        <div class="card">
            <div class="pt-2 pb-2 card-header">
                <h6 class="mb-0 card-title">Product Type</h6>
            </div>
        
            <div class="card-body">
                <div class="form-group{{ $errors->has('product_type') ? ' has-error' : '' }}">
                    {{ html()->label('Product Type', 'product_type') }}
                    {{ html()->select('product_type', [1 => 'Paper', 0 => 'Other'])
        ...
        <div class="card">
            <div class="pt-2 pb-2 card-header">
                <h6 class="mb-0 card-title">Stock</h6>
            </div>
        
            <div class="card-body">
                <div class="form-group{{ $errors->has('stock') ? ' has-error' : '' }}">
                    {{ html()->label('Stock', 'stock') }}
                    {{ html()->select('stock', [1 => 'Low Stock'])
                        ->id('filter_stock')
...
$(function () {  
   $("#categories").jstree({
    ...
    "types": {
        "default" : {
            "icon" : "bx bxs-folder text-warning fs-20" // Closed folder icon by default
        },
        "folder" : {
            "icon" : "bx bxs-folder  text-warning fs-20" // Closed folder icon for folder type
        },
        "folder-opened" : {
            "icon" : "bx bxs-folder-open text-info fs-22" // Opened folder icon for folder type
        }
    },
    "state" : { "key" : "demo2" },
...
function getNodeOrder(nodeId) {
        var node = $('#categories').jstree('get_node', nodeId);
        var order = [];
        node.children.forEach(function (childId) {
            var childNode = $('#categories').jstree('get_node', childId);
            order.push({
                id: childId,
                children: getNodeOrder(childId) 
            });
        });
    ...
    $("#categories").on('move_node.jstree', function (e, data) {
        var newOrder = getNodeOrder('#'); 
        $.ajax({
            url: '{{ route('admin.category.change') }}',
            type: 'POST',
            data: {
    ...
    $('#categories').on('open_node.jstree', function (e, data) {
        data.instance.set_type(data.node, "folder-opened");
    });

    // Event listener to toggle folder icon when closed
    $('#categories').on('close_node.jstree', function (e, data) {
                    ...
                    render: function(data, type, row) {
                        if (type === 'display') {
                            ...
                            @can('ledger_product')
                                 ...
                                 btn+='<li><a class=" dropdown-item edit-item-btn" href="{{ request()->url() }}/rate/' + row['id'] + '"><i class="align-bottom bx bx-rupee fs-16 me-2 text-muted"></i> Check Rate</a></li>';

                                btn+='<li><a class=" dropdown-item edit-item-btn" href="{{ request()->url() }}/ledger/' + row['id'] + '"><i class="align-bottom ri-pencil-fill me-2 text-muted"></i> Ledger</a></li>';
                            @endcan

                            @can('edit_product')
                                btn+='<li><a data-type="'+row['gsm']+'" data-id="'+row['id']+'" class="editProduct dropdown-item edit-item-btn" href="javascript:void(0);"><i class="align-bottom ri-pencil-fill me-2 text-muted"></i> Edit</a></li>';
                            @endcan

                            // @can('delete_product')
                            // btn += '<li><button type="button" onclick="deleteAjax(\''+window.location.href+'/'+row['id']+'/delete\')" class="dropdown-item remove-item-btn"><i class="align-bottom ri-delete-bin-fill me-2 text-muted"></i> Delete</button></li>';
                            // @endcan
